...
Qué hace:
  ...
  2) Crea una pestaña nueva en el proyecto (POST /api/term/new, PIN de ~/.cacho_pin).
     La pestaña arranca sola una sesión interactiva de `claude`.
...
def main():
    ...
    if not ping() and not levantar_server():
        raise SystemExit("El server de Cacho no levanta (ver %s)" % LOG)
    p = pin()

    from urllib.parse import quote
    r = _post("/api/term/new?cwd=%s&pin=%s" % (quote(proyecto), quote(p)))
    tid = r.get("id")
    if not tid:
    ...
    rr = _post("/api/term/%s/input?pin=%s" % (tid, quote(p)),
